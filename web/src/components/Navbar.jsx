    ...
    <header className="bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 border-b border-slate-200">
      <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-2">
          ...
          <select onChange={changeLang} defaultValue={i18n.language} className="border border-slate-300 rounded-md text-sm px-2 py-1" aria-label="Language">
            <option value="en">EN</option>
            <option value="es">ES</option>
          </select>
          <select value={theme} onChange={(e)=>setTheme(e.target.value)} className="border border-slate-300 rounded-md text-sm px-2 py-1" aria-label="Theme">
            <option value="light">Light</option>
            <option value="dark">Dark</option>
          </select>
          <select value={contrast} onChange={(e)=>setContrast(e.target.value)} className="border border-slate-300 rounded-md text-sm px-2 py-1" aria-label="Contrast">
            <option value="normal">Normal</option>
            <option value="high">High</option>
          </select>
          <select value={font} onChange={(e)=>setFont(e.target.value)} className="border border-slate-300 rounded-md text-sm px-2 py-1" aria-label="Font">
